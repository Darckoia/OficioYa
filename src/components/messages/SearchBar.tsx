  ...
  return (
    <div className="surface-card flex flex-col gap-4 p-4 lg:flex-row">
      <input
        ...
        className="min-h-12 flex-1 rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 outline-none focus:border-brand-400"
      />
      <select
        value={category}
        onChange={(event) => onCategoryChange(event.target.value as MessageCategory | 'Todas')}
        className="min-h-12 rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900 outline-none focus:border-brand-400"
      >
